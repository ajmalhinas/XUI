...
/**
 * @(#)XUIImageImpl.java    0.5 18/08/2003
 *
...
/**
 * Rerpresents a XUIImage. This implementation class represents a XUI Image that can be placed in
 * other components.
 *
 * @see org.purnamaproject.xui.helpers.XUIComponentFactory
 * @see org.purnamaproject.xui.component.atomic.XUIImage
 * @version 0.5 18/08/2003
 * @author  Arron Ferguson
 */
public class XUIImageImpl implements XUIImage
{
    /**
     * The native implementation of this XUI image.
     */
    private JImagePanel icon;

    /**
     * The node representation of this component.
     */
    private XUINode imageNode;
    ...
    /**
     * <p>Constructs a new XUIImage implementation and passing it its XML node rather than requiring
     * the XUIImage implementation to build it from scratch. This method is called by the Realizer
     ...
     * This constructor is required when creating a XUI Image where the node has already been built
     ...
     */
    public XUIImageImpl(XUINode node)
    {
        ...
        if(!(node.getName().equals("Image")))
            throw new XUITypeFormatException("Node for XUIImage must be named 'Image' and conform to the XUI schema.");
        else
        {
            List dataList = node.getDirectChildren();
            XUINode data = (XUINode)dataList.get(0);
            // gui component
            ImageIcon i = XUIUtils.base64ToImage(data.getCDATA(), node.getAttributeValue("type"));
            icon = new JImagePanel(i);
    ...
    /**
     * <p>Creates an image with no actual image data. Default size is 1 x 1 cells in the
     * top left corner of any container that its placed in.</p>
     *
     * <p>This constructor should not normally be called as it does not set the image type
     * which is important for marshalling the images out to the XUI document.</p>
     */
    public XUIImageImpl()
    {
        icon = new JImagePanel();

        // create the XML node representing this component.
        // now create the XML version of this element
        imageNode = new XUINodeImpl("Image");
        imageNode.setLevel(4);
        imageNode.addNamespace("xui", "http://xml.bcit.ca/PurnamaProject/2003/xui");
        imageNode.setXUIComponent(this);
    ...
    /**
     * <p>Constructor accepting an image name based on a well-formed and valid URL. Locates and loads the
     * image based on this string.</p>
     *
     * <p><b>Note:</b> the Purnama XUI API only supports JPEG, GIF and PNG file formats. All others types
     * will generate exceptions at run time.</p>
     *
     * @see #setComponentLocation(int, int)
     * @param imageURL the URL to find this image at. Must be a valid URL.
     * @throws org.purnamaproject.xui.XUIDisplayException if the image URL is null.
     */
    public XUIImageImpl(URL imageURL) throws XUIDisplayException
    {
        this();
        ...
        StringTokenizer tokenizer = new StringTokenizer(imageURL.toString(), "/");
        ...
        if(imageName.length() == 0)
            throw new XUIDisplayException("The URL name must be a valid image name.");

        else if(imageNameLowerCase.endsWith("jpeg") || imageNameLowerCase.endsWith("jpg") ||
            imageNameLowerCase.endsWith("gif") || imageNameLowerCase.endsWith("png"))
        {
            ImageIcon imageFromURL = new ImageIcon(imageURL);

            icon = new JImagePanel(imageFromURL);


            // get the image's file type and name
            String type = "";
            ...
            try
            {
                ...
                data.setCDATA(new String(XUIUtils.imageToBase64(icon.getImage())));
            ...
            throw new XUIDisplayException("The Purnama XUI API only supports JPEG, PNG and GIF.");
    }


    /**
     * <p>Constructor accepting an image name based on a string. Locates the image based on this string.
     * No protocol is given and none is assumed. The name of the file must <b>not</b> include any path -
     * only a file name.</p>
     *
     * <p><b>Note:</b> the Purnama XUI API only supports JPEG, GIF and PNG file formats. All others types
     * will generate exceptions at run time.</p>
     ...
     * @throws org.purnamaproject.xui.XUIDisplayException if the image URL is null.
     */
    public XUIImageImpl(URL imageURL, int xCoordinate, int yCoordinate) throws XUIDisplayException
    {
        this(imageURL);
    ...
    {
        return imageNode.getAttributeValue("name");
    }
     ...
     */
    public void setName(String newName) throws XUIDisplayException
    {
        if(newName == null)
            throw new XUIDisplayException("Name for XUIImage cannot be null.");
        else if(newName.length() < 1)
            throw new XUIDisplayException("The length of name must be more than 0 character long.");
        else
        {
            icon.setName(newName);
            imageNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "name", "name", "xs:string",
                icon.getName());
        }
    }
    ...
    {
        return imageNode.getAttributeValue("type");
    }
    ...
    public int getX()
    {
        return Integer.parseInt(imageNode.getAttributeValue("x"));
    }
    ...
    public int getY()
    {
        return Integer.parseInt(imageNode.getAttributeValue("y"));
    }
    ...
    public int getWidth()
    {
        return Integer.parseInt(imageNode.getAttributeValue("width"));
    }
    ...
    {
        return Integer.parseInt(imageNode.getAttributeValue("height"));
    }

    /**
     * Sets this component to being enabled. By being enabled, it will respond to user operations.
     * For this component, it does nothing since there is nothing to enable in an image.
     *
     ...
     *
     * <p>This method is normally not something that needs to be called. It is a method that is called
     * by the parent container adding the sub-container, atomic or composite component. API users do not
     * need to be aware of this method.</p>
     *
     * <p><b>Note:</b> <i>Since XUIImage can be placed within a container (XUIPanel) and non containers (i.e.
     * atomic components), This method will behave differently under those two different circumstances.
     * The x, y, width and height will be honoured and used if placed within the container. If however
     * the XUIImage is placed within an atomic component, the coordinates and size will not be ignored
     * and the default values set to: x=0, y=0, width=1, height=1. This should have no affect on
     * any placement of XUIImages whether they are in atomic components <b>or</b> the XUIPanel container.</i></p>
     *
        ...
        throws XUIDisplayException
    {
        ...
        else
        {
            ...
            XUINode parentNode = imageNode.getParent();
            if(parentNode != null)
            ...
            if(parent != null)
            {
                ...
                if(!(parentName.equals("Button")) && !(parentName.equals("RadioButton"))
                && !(parentName.equals("CheckBox")) && !(parentName.equals("MenuItem")))
                {
                    // collision detection
                    ...
                    while(keys.hasMoreElements())
                    {
                        ...
                        if(region.intersects(spaceToOccupy) && someComponent != this)
                            throw new XUIDisplayException("Component: " + someComponent + " is already in the area x: "
                    ...
                    else
                    {
                        ...
                        container.add(icon, new Rectangle(gridY, gridX, gridWidth, gridHeight));
                        icon.invalidate();
                        container.validate();
