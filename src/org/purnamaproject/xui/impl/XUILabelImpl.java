...
/**
 * @(#)XUILabelImpl.java    0.5 18/08/2003
 *
 ...
 */
public class XUILabelImpl implements XUILabel
{
    /**
    ...
    /**
     * <p>Constructs a new XUILabel implementation and passing it its XML node rather than requiring
     * the XUILabel implementation to build it from scratch. This method is called by the Realizer
     ...
     * This constructor is required when creating a XUI Label where the node has already been built
     ...
     */
    public XUILabelImpl(XUINode node) throws XUIDisplayException
    {
        ...
        if(!(node.getName().equals("Label")))
            throw new XUIDisplayException("Node for XUILabel must be named 'Label' and conform to the XUI schema.");
        else
        {
            // gui component
            label = new JLabel();
            ...
            labelNode = node;
            labelNode.setXUIComponent(this);

            // justification
            String justifyMe = labelNode.getAttributeValue("justified");
            String text = labelNode.getAttributeValue("text");
            if(justifyMe.equals("right"))
            {
                label.setHorizontalAlignment(SwingConstants.RIGHT);
                label.setText(text);

            } else if(justifyMe.equals("left"))
            {
                label.setHorizontalAlignment(SwingConstants.LEFT);
                label.setText(text);

            } else if(justifyMe.equals("center"))
            {
    ...
    /**
     * Creates a label. Default size is 1 x 1 cells in the top left corner.
     */
    public XUILabelImpl()
    {
        // gui component
        ...
        labelNode = new XUINodeImpl("Label");
        labelNode.setLevel(4);
        labelNode.addNamespace("xui", "http://xml.bcit.ca/PurnamaProject/2003/xui");
        labelNode.setXUIComponent(this);

        // add specific attributes
        labelNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "id", "id", "xs:ID",
            IDFactory.getInstance().generateID("label"));
        labelNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "x", "x", "xs:unsignedShort",
            "0");
        labelNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "y", "y", "xs:unsignedShort",
            "0");
        labelNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "width", "width", "xs:unsignedShort",
            "1");
        labelNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "height", "height", "xs:unsignedShort",
            "1");
        labelNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "justified", "justified",
            "xs:token", "left");
        labelNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "text", "text",
            "xs:string", "");

    }

    /**
     * Creates a label and gives it x and y coordinates. If the label has a parent,
     * then the label will be placed at that position. The x and y values must be within the XUI
     ...
     */
    public XUILabelImpl(int xCoordinate, int yCoordinate) throws XUIDisplayException
    {
        this();
    ...
    /**
     * Creates a label and gives it x and y coordinates with text. If the label has a parent,
     * then the label will be placed at that position. The x and y values must be within the XUI
     * schema specification as well, be placed on the panel and not overtop of any other component.
     * If so, an exception is generated.
     *
     ...
     */
    public XUILabelImpl(int xCoordinate, int yCoordinate, String text) throws XUIDisplayException
    {
        ...
        {
            labelNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "text", "text", "xs:string",
                text);
            label.setText(text);
        }
    }
    ...
    public void setEnabled(boolean enabled)
    {
        ...
        labelNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "enabled", "enabled", "xs:boolean",
            "" + label.isEnabled());
    }
    ...
    {
        return labelNode.getAttributeValue("text");
    }
    ...
    {
        if(newText == null)
            throw new XUITypeFormatException("string for a label cannot be null.");
    ...
    {
        if(justification > 2 || justification < 0)
            return;
        String j = "";
        if(justification == XUIUtils.LEFT_JUSTIFIED)
        {
            j = "left";
            label.setHorizontalAlignment(JLabel.LEFT);
        }
        else if(justification == XUIUtils.RIGHT_JUSTIFIED)
        {
            j = "right";
            label.setHorizontalAlignment(JLabel.RIGHT);
        }
        else if(justification == XUIUtils.CENTER_JUSTIFIED)
        {
            j = "center";
    ...
    public String getID()
    {
        return labelNode.getAttributeID();
    }
    ...
    public void setComponentLocation(int xCoordinate, int yCoordinate) throws XUIDisplayException
    {
        ...
        else
        {
            ...
            XUINode parentNode = labelNode.getParent();
            if(parentNode != null)
            ...
            if(parent != null)
            {
                ...
                else
                {
                    ...
                    container.add(label, new Rectangle(gridY, gridX, gridWidth, gridHeight));
                    label.invalidate();
                    container.validate();
