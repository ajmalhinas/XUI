...
/**
 * @(#)XUIListImpl.java 0.5 18/08/2003
 *
 ...
 */
public class XUIListImpl implements XUIList, ListSelectionListener, MouseListener, KeyListener
{
    /**
    ...
    /**
     * The scroll pane that this list fits into so as to give
     * the feature of scrolling.
     */
     ...
     * component.
     */
    private List listActionModelList = new LinkedList();

    /**
     * <p>Constructs a new XUIList implementation and passing it its XML node rather than requiring
     * the XUIList implementation to build it from scratch. This method is called by the Realizer
     ...
     * This constructor is required when creating a XUI list where the node has already been built
     ...
     */
    public XUIListImpl(XUINode node) throws XUIDisplayException
    {
        ...
        if(!(node.getName().equals("List")))
            throw new XUIDisplayException("Node for XUILIst must be named 'List' and conform to the XUI schema.");
        else
        {
            // gui component
            ...
            // deal with scrolling
            if(node.getAttributeValue("scrolling").equals("horizontal"))
            {
                scroller.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_NEVER);
                scroller.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);

            } else if(node.getAttributeValue("scrolling").equals("vertical"))
            {
                scroller.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
                scroller.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);

            } if(node.getAttributeValue("scrolling").equals("both"))
            {
                scroller.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
                scroller.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);

            } if(node.getAttributeValue("scrolling").equals("none"))
            {
                scroller.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_NEVER);
                scroller.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
            ...
            // add
            listNode = node;
            listNode.setXUIComponent(this);

            list.setEnabled(Boolean.valueOf(node.getAttributeValue("enabled")).booleanValue());
            // only try this if there are values.
            if(Integer.parseInt(node.getAttributeValue("itemSelected")) > -1)
                list.setSelectedIndex(Integer.parseInt(node.getAttributeValue("itemSelected")));

            // get the list items
            List items = node.getChildNodesByName("ListItem");
            for(int i = 0; i < items.size(); i++)
            {
                XUINode item = (XUINode)items.get(i);
                int pos = list.getModel().getSize();
                model.add(pos, item.getCDATA());
            }
        }
    }

    /**
     * Creates a list with no items. Default size is 1 x 1 cells in the top left corner with both
    ...
    public XUIListImpl()
    {
        ...
        listNode = new XUINodeImpl("List");
        listNode.setLevel(4);
        listNode.addNamespace("xui", "http://xml.bcit.ca/PurnamaProject/2003/xui");
        listNode.setXUIComponent(this);


        // add specific attributes
        listNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "id", "id", "xs:ID",
            IDFactory.getInstance().generateID("list"));
        listNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "x", "x", "xs:unsignedShort",
            "0");
        listNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "y", "y", "xs:unsignedShort",
            "0");
        listNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "width", "width", "xs:unsignedShort",
            "1");
        listNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "height", "height", "xs:unsignedShort",
            "1");
        listNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "enabled", "enabled", "xs:boolean",
            "true");
        listNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "scrolling", "scrolling", "xs:token",
            "vertical");
        listNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "itemSelected", "itemSelected",
            "xs:unsignedShort", "0");

    }

    /**
     * Creates a list and gives it x and y coordinates. If the list has a parent,
     * then the list will be placed at that position. The x and y values must be within the XUI
     ...
     */
    public XUIListImpl(int xCoordinate, int yCoordinate) throws XUIDisplayException
    {
        this();
    ...
    /**
     * Creates a list and gives it x and y coordinates and scrolling. If the list has a parent,
     * then the list will be placed at that position. The x and y values must be within the XUI
     * schema specification as well, be placed on the panel and not overtop of any other component.
     * If so, an exception is generated.
     *
     ...
     * @param xCoordinate the x coordinate used to place the button on the grid.
     * @param yCoordinate the y coordinate used to place the button on the grid.
     * @param scrollChoice the scrolling choice of either horizontal scroll, vertical scroll, both or none.
     */
    public XUIListImpl(int xCoordinate, int yCoordinate, int scrollChoice) throws XUIDisplayException
    {
        this(xCoordinate, yCoordinate);

        if(scrollChoice == XUIUtils.HORIZONTAL_SCROLLING)
        {
            scroller.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_NEVER);
            scroller.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);

            listNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "scrolling", "scrolling",
                "xs:unsignedShort", "horizontal");


        } else if(scrollChoice == XUIUtils.VERTICAL_SCROLLING)
        {
        ...
        } if(scrollChoice == XUIUtils.BOTH_SCROLLING)
        {
            scroller.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED);
            scroller.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);

            listNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "scrolling", "scrolling",
                "xs:unsignedShort", "both");


        } if(scrollChoice == XUIUtils.NO_SCROLLING)
        {
            scroller.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_NEVER);
            scroller.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
    ...
    public void addNewItem(String text) throws XUITypeFormatException
    {
        ...
        {
            // another convoluted Java Swing stupidity:
            int pos = list.getModel().getSize();
            model.add(pos, text);
            // end flame >:^)

            XUINode itemNode = new XUINodeImpl("ListItem");
            int newLevel = listNode.getLevel();
            itemNode.setLevel(newLevel);
            itemNode.addNamespace("xui", "http://xml.bcit.ca/PurnamaProject/2003/xui");
            // add to the combo box node
            listNode.addChildNode(itemNode);
        }
    }
    ...
    public void setSelectedIndex(int index) throws XUITypeFormatException
    {
        ...
        else
        {
            ...
            listNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "itemSelected",
                "itemSelected", "xs:unsignedShort", "" + index);
        }
    }
    ...
    public void deleteItem(int index) throws XUITypeFormatException
    {
        ...
        {
            model.remove(index);
            listNode.removeChildNode(index);
        }
    }
    ...
    public String getID()
    {
        return listNode.getAttributeID();
    }
    ...
    public void setComponentLocation(int xCoordinate, int yCoordinate) throws XUIDisplayException
    {
        ...
        else
        {
            ...
            XUINode parentNode = listNode.getParent();
            if(parentNode != null)
            ...
            if(parent != null)
            {
                ...
                else
                {
                    ...
                    container.add(scroller, new Rectangle(gridY, gridX, gridWidth, gridHeight));
                    scroller.invalidate();
                    container.validate();
    ...
    {
        // get the ListModel from the JList, cast it to a DefaultListModel,
        // get its items as an Enumeration, pass the Enumeration to the static
        // Collections list method which takes an Enumeration and spits out
        // a List. Bit of a pain.
        return Collections.list(((DefaultListModel)list.getModel()).elements());
    }
    ...
    {
        if(model instanceof ListActionModel)
        {
            // call up the action model.
            listActionModelList.add(model);
    ...
    {
        listActionModelList.remove(model);
    ...
    /**
     * Common code that tells all ListActionModel listeners
     * about what just happened. This method is called by several
     * implementation specific event handling methods.
     *
     * @param type the type of event that just took place.
     * @param index the index of the component.
     */
    private void handleListActionListeners(byte type, int index)
    {
        int size = listActionModelList.size();
        for(int i = 0; i < size; i++)
        {
            ListActionModel model = (ListActionModel)listActionModelList.get(i);
            model.listAction(this, type, index);
        }
    }
    ...
    /**
     * The delete key was pressed and the item is being deleted.
     */
    public void keyReleased(KeyEvent e)
    {
        if(e.getSource() == list)
        {
            if(e.getKeyCode() == KeyEvent.VK_DELETE)
            {
                ...
                handleListActionListeners(ListActionModel.DELETED ,index);
            }
        }
    }

    /**
     * There was a double click on this component.
     *
     * @param e the event
     */
    public void mouseClicked(MouseEvent e)
    {
        if(e.getSource() == list)
        {
            ...
            {
                handleListActionListeners(ListActionModel.DOUBLE_CLICK ,list.getSelectedIndex());
            }
        }
    }

    /**
     * There was a selection change made by the native implementation. This handles
    ...
    {
        // check first if its ours
        if(e.getSource() == list)
        {
            // this is a selection ... single click
            if (!e.getValueIsAdjusting())
            {
                JList list = (JList)e.getSource();
                handleListActionListeners(ListActionModel.SINGLE_CLICK ,list.getSelectedIndex());
            }
