...
/**
 * @(#)XUISplitPanelImpl.java   0.5 18/08/2003
 *
...
/**
 * The implementation of XUISplitPanel.
 *
 * @version 0.5 18/08/2003
 * @author  Arron Ferguson
 */
public class XUISplitPanelImpl implements XUISplitPanel
{
    /**
     ...
     */
    private XUINode splitNode = null;
    ...
    /**
     * <p>Constructs a new XUISplitPanel implementation and passing it its XML node rather
     * than requiring the XUISplitPanel implementation to build it from scratch. This
     ...
     * This constructor is required when creating a XUI SplitPanel where the node has already been built
     ...
     */
    public XUISplitPanelImpl(XUINode node)
    {
        ...
        if(!(node.getName().equals("SplitPanel")))
            throw new XUIDisplayException("Node for XUISplitPanel must be named 'SplitPanel' and conform to the XUI schema.");
        else
        {
            // gui component
            splitNode = node;
            ...
            if(node.getAttributeValue("orientation").equals("horizontal"))
            {
            ...
            float divider = Float.parseFloat(node.getAttributeValue("leftTopPercentage"));
            divider *= 0.01;
            panel.setDividerLocation(divider);
            panel.setResizeWeight(divider);
            panel.setOneTouchExpandable(true);
            // add as XUI component
            splitNode.setXUIComponent(this);
    ...
    /**
     * Default SplitPanel with horizontal orientation, created with a size of 1 x 1 cells within the
     * layout in the top left corner of the container.
     */
    public XUISplitPanelImpl()
    {
        panel = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT);
        ...
        splitNode = new XUINodeImpl("SplitPanel");
        splitNode.setLevel(2);
        splitNode.addNamespace("xui", "http://xml.bcit.ca/PurnamaProject/2003/xui");
        splitNode.setXUIComponent(this);

        // add window specific attributes
        splitNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "id", "id", "xs:ID",
            IDFactory.getInstance().generateID("panel"));
        splitNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "x", "x", "xs:unsignedShort",
            "0");
        splitNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "y", "y", "xs:unsignedShort",
            "0");
        splitNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "width", "width", "xs:unsignedShort",
            "1");
        splitNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "height", "height", "xs:unsignedShort",
            "1");
        splitNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "leftTopPercentage",
            "leftTopPercentage", "xs:unsignedShort", "50");
        splitNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "orientation",
            "orientation", "xs:token", "horizontal");

    }

    /**
     * Constructor for creating a new XUISplitPanel component. Component will be centered using the width
     * and height given.
     *
     * @param width the width of this XUISplitPanel.
     * @param height the height of this XUISplitPanel.
     */
    public XUISplitPanelImpl(int width, int height) throws XUIDisplayException
    {
        this();
     ...
     */
    public XUISplitPanelImpl(int width, int height, int orientation) throws XUIDisplayException
    {
        this(width, height);
    ...
    public void removeAllComponents()
    {
        ...
        List list = splitNode.getAllChildNodes();
     ...
     *
     * <p><b>This method adds the component either to the top or the left pane. If there is currently
     * a component in place, it will be removed and this new component will take the old one's place.</b></p>
     *
    ...
    {
        addComponent(component, XUIUtils.PLACEMENT_LEFT_OR_TOP);
    }
    ...
    public void removeComponent(XUIComponent component)
    {
        ...
        for(int i = 0; i < list.size(); i++)
        {
            ...
            if(c == component)
            {
                ...
                splitNode.removeChildNode(node);
            }
        }
    }
    ...
    {
        if(orientation == XUIUtils.ORIENT_HORIZONTAL)
        {
            panel.setOrientation(JSplitPane.HORIZONTAL_SPLIT);
            splitNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "orientation", "orientation",
                "xs:unsignedShort", "horizontal");

        } else if(orientation == XUIUtils.ORIENT_VERTICAL)
        {
            panel.setOrientation(JSplitPane.VERTICAL_SPLIT);
            splitNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "orientation", "orientation",
                "xs:unsignedShort", "vertical");

        } else
            throw new XUITypeFormatException("Type must be either horizontal or vertical.");
    }
    ...
    public void setDivide(float percentage) throws XUITypeFormatException
    {
        if(percentage < 0 || percentage > 100)
            throw new XUITypeFormatException("The percentage must be: >= 0 <= 100.");
        else
        {
            float divider = percentage *  0.01f;
            panel.setDividerLocation((double)divider);
            splitNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "leftTopPercentage",
                "leftTopPercentage", "xs:unsignedShort", ((int)percentage) + "");
        }
    }

    /**
     * Adds a new component to this container in either top, bottom, left or right. If the layout chosen is
     * top and bottom, then placement values must be either PLACEMENT_LEFT_OR_TOP or PLACEMENT_RIGHT_OR_BOTTOM.
     * An exception will be generated otherwise.
     *
     * <p><b>Note: This method calls up the specific class's method and uses the default position of
     * XUIUtils#PLACEMENT_LEFT_OR_TOP or XUIUtils#PLACEMENT_RIGHT_OR_BOTTOM based on the orientation of this
     * split panel.</b></p>
     *
     * @throws org.purnamaproject.xui.XUITypeFormatException if the type does not conform to the
     * <a href="http://geekkit.bcit.ca/xui/docs/specification/xuiSpecification0.5.0.html">Purnama Project XUI Specification</a>.
     * @param component the component to add.
     * @param placement the placement of this component. It must be one of the constants found in the XUIUtils
     * class.
     ...
     */
    public void addComponent(XUIComponent component, int placement) throws XUITypeFormatException
    {
        if(component == null)
            throw new XUITypeFormatException("Component must not be null.");
        else if(placement != XUIUtils.PLACEMENT_LEFT_OR_TOP && placement != XUIUtils.PLACEMENT_RIGHT_OR_BOTTOM)
            throw new XUITypeFormatException("Component must not be null.");
        ...
        else
        {
            ...
            int splitPanelOrientation = panel.getOrientation();
            if(splitPanelOrientation == JSplitPane.HORIZONTAL_SPLIT)
            {
                if(placement == XUIUtils.PLACEMENT_LEFT_OR_TOP)
                    panel.setLeftComponent(component.getPeer());
                else if(placement == XUIUtils.PLACEMENT_RIGHT_OR_BOTTOM)
                    panel.setRightComponent(component.getPeer());
            } else
            {
                if(placement == XUIUtils.PLACEMENT_LEFT_OR_TOP)
                    panel.setTopComponent(component.getPeer());
                else if(placement == XUIUtils.PLACEMENT_RIGHT_OR_BOTTOM)
                    panel.setBottomComponent(component.getPeer());
            }
    ...
    {
        return splitNode.getAttributeValue("idref");
    }
    ...
    public void setIDRef(String newIDRef) throws XUITypeFormatException
    {
        ...
        if(newIDRef.matches("[^a-zA-Z]"))
            splitNode.addAttribute("http://xml.bcit.ca/PurnamaProject/2003/xui", "idref", "idref", "xs:IDREF",
                newIDRef);
        else
    ...
    public String getID()
    {
        return splitNode.getAttributeValue("id");
    }
    ...
    public void setComponentLocation(int xCoordinate, int yCoordinate) throws XUIDisplayException
    {
        ...
        else
        {
            ...
            XUINode parentNode = splitNode.getParent();
